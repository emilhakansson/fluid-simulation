                              ...
                              unsigned int const vertical_split_count)
{
	...
	unsigned int vertices_nb = vertical_vertices_count * horizontal_vertices_count;

	auto vertices = std::vector<glm::vec3>(vertices_nb);
	auto texcoords = std::vector<glm::vec3>(vertices_nb);
	size_t index = 0u;
	
	float const d_width = width / vertical_edges_count;
	float const d_height = height / horizontal_edges_count;
	float current_width = 0, current_height = 0;

	// Generate vertices

	for (unsigned int i = 0; i < horizontal_vertices_count; ++i) {
		current_width = 0;
		for (unsigned int j = 0; j < vertical_vertices_count; ++j) {
			vertices[index] = glm::vec3(current_width, 0.0f, current_height);
			texcoords[index] = glm::vec3(current_width / width, current_height / height, 0.0f);
	...
	index = 0u;
	unsigned int indices_nb = (2u * horizontal_edges_count * vertical_edges_count);
	auto index_sets = std::vector<glm::uvec3>(indices_nb);

	for (unsigned int i = 0; i < horizontal_edges_count; ++i) {
		for (unsigned int j = 0; j < vertical_edges_count; ++j) {
			index_sets[index] = glm::uvec3(
				vertical_vertices_count * (i + 0u) + (j + 0u),
				vertical_vertices_count * (i + 1u) + (j + 1u),
				vertical_vertices_count * (i + 0u) + (j + 1u));
			++index;
			index_sets[index] = glm::uvec3(
				vertical_vertices_count * (i + 0u) + (j + 0u),
				vertical_vertices_count * (i + 1u) + (j + 0u),
				vertical_vertices_count * (i + 1u) + (j + 1u));
			++index;
		}
	}
...
bonobo::mesh_data
parametric_shapes::createInstancedQuad(int instances_nb)
{
	bonobo::mesh_data data;
...
bonobo::mesh_data
parametric_shapes::createBox(float const width, float const height, float const depth) {
	auto const vertices_nb = 8;
	auto vertices = std::vector<glm::vec3>(vertices_nb);

	vertices = { glm::vec3(-width / 2, -height / 2,  depth / 2),
	...
	auto index_sets = std::vector<glm::uvec2>(12);
	index_sets = {
		glm::vec2(0, 1),
		glm::vec2(0, 2),
		glm::vec2(0, 5),
	...
	data.indices_nb = static_cast<GLsizei>(index_sets.size() * 2u);
	glGenBuffers(1, &data.ibo);
	assert(data.ibo != 0u);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, data.ibo);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, static_cast<GLsizeiptr>(index_sets.size() * sizeof(glm::uvec2)), reinterpret_cast<GLvoid const*>(index_sets.data()), GL_STATIC_DRAW);
                                ...
                                unsigned int const latitude_split_count)
{
	...
	//set up constants
	float const d_theta = glm::two_pi<float>() / longitude_slice_edges_count;
	float const d_phi = glm::pi<float>() / latitude_slice_edges_count;

	float theta = 0.0f, phi = 0.0f;
	size_t index = 0u;

	// generate vertices from parametric equations
	for (unsigned int i = 0u; i < longitude_slice_vertices_count; ++i) {
		...
		for (unsigned int j = 0u; j < latitude_slice_vertices_count; ++j) {
			...
			texcoords[index] = glm::vec3(
				static_cast<float>(i) / static_cast<float>(longitude_slice_vertices_count - 1),
				static_cast<float>(j) / static_cast<float>(latitude_slice_vertices_count - 1),
				0.0f);	
                               ...
                               unsigned int const minor_split_count)
{
	...
	unsigned int const major_edges_count = major_split_count + 1u;
	unsigned int const minor_edges_count = minor_split_count + 1u;
	unsigned int const major_vertices_count = major_edges_count + 1u;
	unsigned int const minor_vertices_count = minor_edges_count + 1u;

	unsigned int vertices_nb = major_vertices_count * minor_vertices_count;
	...
	float const d_theta = glm::two_pi<float>() / major_edges_count;
	float const d_phi = glm::two_pi<float>() / minor_edges_count;
	float theta = 0.0f, phi = 0.0f;

	size_t index = 0u;

	for (int i = 0; i < major_vertices_count; ++i) {
		phi = 0.0f;
		float sin_theta = glm::sin(theta);
		float cos_theta = glm::cos(theta);
		for (int j = 0; j < minor_vertices_count; ++j) {
			...
			binormals[index] = glm::vec3(
				-(major_radius + minor_radius * cos_theta) * sin_phi,
				0,
				(major_radius + minor_radius * cos_theta) * cos_phi);

			normals[index] = glm::cross(binormals[index], tangents[index]);

			texcoords[index] = glm::vec3(
				static_cast<float>(j) / static_cast<float>(minor_vertices_count - 1),
				static_cast<float>(i) / static_cast<float>(major_vertices_count - 1),
				0.0f);
	...
	for (int i = 0; i < major_edges_count; ++i) {
		for (int j = 0; j < minor_edges_count; ++j) {
			index_sets[index] = glm::uvec3(
				minor_vertices_count * (i + 0u) + (j + 0u),
