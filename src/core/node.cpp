...
void
Node::renderInstanced(int instances_nb, glm::mat4 const& view_projection, glm::mat4 const& parent_transform) const
{
	if (_program != nullptr)
		renderInstanced(instances_nb, view_projection, parent_transform * _transform.GetMatrix(), *_program, _set_uniforms);
}

void
Node::renderInstanced(int instances_nb, glm::mat4 const& view_projection, glm::mat4 const& world, GLuint program, std::function<void(GLuint)> const& set_uniforms) const {
	if (_vao == 0u || program == 0u)
		return;
	...
	glBindVertexArray(_vao);
	glDrawArraysInstanced(GL_TRIANGLES, 0, 6, instances_nb);
	glBindVertexArray(0u);
